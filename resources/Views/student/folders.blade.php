    ...
    <div class="mb-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 bg-surface border border-borderCol p-8 sm:p-10 rounded-3xl shadow-darkCard relative">
            ...
            <div class="shrink-0 z-10">
                <a href="{{ route('landing') }}" class="btn-outline inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold shadow-md transition-all">
                    &larr; Back to Home
                </a>
            </div>
    ...
    <div class="mb-12">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($folders as $folder)
                <a href="{{ route('student.report.show', ['id' => $folder->id, 'type' => 'daily']) }}" class="bg-surface border border-borderCol hover:border-orangeAccent rounded-3xl p-6 transition-all duration-300 shadow-darkCard hover:-translate-y-1 flex flex-col justify-between group block">
                    <div>
    ...
    <div class="mb-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($subjects as $subject)
                <a href="{{ route('student.report.show', ['id' => $subject->id, 'type' => 'daily']) }}" class="bg-surface border border-borderCol hover:border-orangeAccent rounded-3xl p-6 transition-all duration-300 shadow-darkCard hover:-translate-y-1 flex flex-col justify-between group block">
                    <div>
                        <div class="flex justify-between items-start mb-4 gap-3">
                            <div class="flex items-center gap-3.5 min-w-0">
                                <div class="w-12 h-12 rounded-2xl border border-orangeAccent/40 bg-orangeAccent/10 flex items-center justify-center text-2xl flex-shrink-0 shadow-inner">
                                ...
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2">
                                        @if(isset($subject->today_attendance_marked) && $subject->today_attendance_marked)
                                            <span class="w-3 h-3 rounded-full bg-emerald-500 shadow-md flex-shrink-0" title="Today's attendance marked"></span>
                                        @else
                                            <span class="w-3 h-3 rounded-full bg-red-500 shadow-md flex-shrink-0" title="Today's attendance pending"></span>
                                        @endif
