...
<div class="max-w-[92rem] mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    {{-- ===================================================== --}}
    {{-- PAGE HEADER (BACK TO FOLDER OVERVIEW & ADD STUDENT) --}}
    {{-- ===================================================== --}}
    <div class="mb-8">
        ...
        <div class="overflow-x-auto">
            <table class="table-dark text-left text-base min-w-[1000px]" id="studentsTable">
                ...
                <tbody id="studentTableBody">
                    @forelse($folder->students as $index => $student)
                        <tr
                        ...
                        >
                            ...
                            <td class="py-5 px-6 text-center">
                                <div class="flex items-center justify-center gap-3">
                                    {{-- Calendar button: Student Added Date --}}
                                    <button
                                        type="button"
                                        ...
                                        onclick="openStudentDateModal(
                                            ...
                                            @js($student->created_at ? $student->created_at->format('d/m/Y') : 'Date not available'),
                                            @js($student->created_at ? $student->created_at->format('h:i A') : '')
                                        )"
                                    ...
                                    <button
                                        type="button"
                                        onclick="openDeleteModal(@js($student->id))"
...
<div id="addModal" class="hidden fixed inset-0 bg-black/85 backdrop-blur-md items-center justify-center z-50 p-4 overflow-y-auto">
    <div class="bg-surface border border-borderCol rounded-3xl p-8 w-full max-w-lg shadow-darkCard">
        ...
        <form id="addStudentForm" method="POST" action="{{ route('admin.students.store', $folder->id) }}" class="space-y-5" onsubmit="return validateStudentForm(event, this)">
            @csrf
