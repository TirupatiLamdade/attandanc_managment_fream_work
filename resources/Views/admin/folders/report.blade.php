...
<div class="max-w-[92rem] mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    {{-- ===================================================== --}}
    {{-- FLOATING TOP NOTIFICATION MESSAGE (AUTO DISMISS AFTER 5 SECONDS) --}}
    {{-- ===================================================== --}}
    @if(session('success'))
    ...
    {{-- ===================================================== --}}
    {{-- CLEAN HEADER: BACK BUTTON (1 STEP BACK), FOLDER ICON & NAME --}}
    {{-- ===================================================== --}}
    <div class="mb-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 bg-surface border border-borderCol p-8 sm:p-10 rounded-3xl shadow-darkCard">
            <div class="flex flex-col sm:flex-row sm:items-center gap-5">
                ...
                <h1 class="text-3xl sm:text-4xl font-extrabold text-textPrimary tracking-tight break-words flex items-center gap-3">
                    ...
                    <span>{{ $folder->name }} <span class="text-textMuted font-medium text-xl sm:text-2xl">/ Attendance Report</span></span>
                </h1>
            </div>
        </div>
    </div>
    ...
    <div class="bg-surface border border-borderCol p-3 rounded-2xl mb-8 shadow-darkCard flex flex-wrap items-center gap-3">
        <a
            href="{{ route('admin.report.show', ['id' => $folder->id, 'type' => 'daily', 'date' => $date]) }}"
        ...
        <a
            href="{{ route('admin.report.show', ['id' => $folder->id, 'type' => 'monthly', 'month' => $month]) }}"
        ...
        <a
            href="{{ route('admin.report.show', ['id' => $folder->id, 'type' => 'custom', 'start' => $start, 'end' => $end]) }}"
        ...
        <a
            href="{{ route('admin.report.show', ['id' => $folder->id, 'type' => 'all']) }}"
    ...
    <div class="bg-surface border border-borderCol p-6 sm:p-8 rounded-3xl mb-8 shadow-darkCard">
        <form method="GET" action="{{ route('admin.report.show', ['id' => $folder->id, 'type' => 'daily']) }}" class="flex flex-col lg:flex-row items-end lg:items-center justify-between gap-5">
            ...
            <div>
                <a
                    href="{{ route('admin.report.pdf', ['id' => $folder->id, 'type' => 'daily', 'date' => $date]) }}"
                    target="_blank"
    ...
    @if($type === 'monthly')
    <div class="bg-surface border border-borderCol p-6 sm:p-8 rounded-3xl mb-8 shadow-darkCard">
        <form method="GET" action="{{ route('admin.report.show', ['id' => $folder->id, 'type' => 'monthly']) }}" class="flex flex-col lg:flex-row items-end lg:items-center justify-between gap-5">
            ...
            <div>
                <a
                    href="{{ route('admin.report.pdf', ['id' => $folder->id, 'type' => 'monthly', 'month' => $month]) }}"
                    target="_blank"
                    class="btn-danger px-6 py-3 rounded-xl font-semibold text-sm inline-flex items-center gap-2 shadow-md w-full sm:w-auto justify-center"
                >
                    <span>📄</span> Download PDF
    ...
    @if($type === 'custom')
    <div class="bg-surface border border-borderCol p-6 sm:p-8 rounded-3xl mb-8 shadow-darkCard">
        <form method="GET" action="{{ route('admin.report.show', ['id' => $folder->id, 'type' => 'custom']) }}" class="flex flex-col lg:flex-row items-end lg:items-center justify-between gap-5" id="customReportForm">
            ...
            <div class="flex items-center gap-3 w-full lg:w-auto justify-end pt-2">
                ...
                @if($hasAttendance)
                <a
                    href="{{ route('admin.report.pdf', ['id' => $folder->id, 'type' => 'custom', 'start' => $start, 'end' => $end]) }}"
                    target="_blank"
    ...
    <div class="bg-surface border border-borderCol p-6 sm:p-8 rounded-3xl mb-8 shadow-darkCard flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div>
            <a
                href="{{ route('admin.report.pdf', ['id' => $folder->id, 'type' => 'all']) }}"
                target="_blank"
                class="btn-danger px-6 py-3 rounded-xl font-semibold text-sm inline-flex items-center gap-2 shadow-md justify-center"
            >
                <span>📄</span> Download PDF
    ...
    {{-- ===================================================== --}}
    {{-- COMPACT SEARCH FILTER BAR WITH ICON --}}
    {{-- ===================================================== --}}
        ...
        <div class="overflow-x-auto">
            <table class="table-dark text-left text-base" id="studentsTable">
                ...
                <tbody id="reportTableBody">
                    ...
                    <tr
                        class="report-row"
                        data-search="{{ strtolower($data['name'] . ' ' . $data['roll'] . ' ' . $data['branch'] . ' ' . $data['phone']) }}"
                    >
