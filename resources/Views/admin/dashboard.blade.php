...
@endif
@if(session('error'))
    <div class="bg-danger/10 border border-danger text-danger px-4 py-2 rounded mb-4">{{ session('error') }}</div>
@endif
<div class="bg-surface border border-border rounded-xl p-6 mb-8 glow-accent">
    ...
    <form method="POST" action="{{ route('admin.folders.store') }}" class="flex gap-4">
        @csrf
...
<div class="mb-8">
    ...
    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-surface border border-border rounded-xl p-4 card-hover transition-all">
            <h3 class="font-semibold text-lg mb-2">{{ $folder->name }}</h3>
            <div class="flex gap-2 mb-3">
                <a href="{{ route('admin.folders.show', $folder->id) }}" class="text-accent hover:underline text-sm">Open →</a>
            </div>
            <div class="flex gap-2">
                <button onclick="editFolder({{ $folder->id }}, '{{ $folder->name }}')" class="text-secondary hover:text-accent text-sm">Edit Name</button>
                <button onclick="deleteFolder({{ $folder->id }})" class="text-danger hover:text-danger text-sm">Delete</button>
            </div>
        </div>
        @endforeach
    </div>
...
<div>
    ...
    <div class="bg-surface border border-border rounded-xl p-6 mb-4">
        <form method="POST" action="{{ route('admin.subjects.store') }}" class="flex gap-4">
            @csrf
    ...
    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <a href="{{ route('admin.subjects.show', $subject->id) }}" class="bg-surface border border-border rounded-xl p-4 card-hover transition-all block">
