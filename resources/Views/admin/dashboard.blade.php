...
<div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="mb-12">
        ...
        <div id="createFolderBox" class="hidden bg-card border border-orangeAccent/50 rounded-3xl p-6 mb-6 shadow-xl relative overflow-hidden transition-all">
            ...
            <form method="POST" action="{{ route('admin.folders.store') }}" class="flex flex-col sm:flex-row gap-3">
                @csrf
                <input
                    type="text"
                    name="name"
            ...
            @forelse($folders as $folder)
                <div class="bg-card border border-borderCol rounded-3xl p-6 transition-all duration-300 hover:border-orangeAccent hover:shadow-2xl hover:-translate-y-1 flex flex-col justify-between group shadow-xl">
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-borderCol text-xs">
                        <button type="button" onclick="editFolder({{ $folder->id }}, @js($folder->name))" class="text-textSecondary hover:text-textPrimary font-medium">
                            Edit Name
                        </button>
                        <button type="button" onclick="deleteFolder({{ $folder->id }})" class="text-red-400 hover:text-red-300 font-medium">
                            Delete
                        </button>
                    </div>
                </div>
    ...
    <div class="mb-8">
        ...
        <div id="createSubjectBox" class="hidden bg-card border border-orangeAccent/50 rounded-3xl p-6 mb-6 shadow-xl relative overflow-hidden transition-all">
            ...
            <form method="POST" action="{{ route('admin.subjects.store') }}" class="flex flex-col sm:flex-row gap-3">
                @csrf
                <input
                    type="text"
                    name="name"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($subjects as $subject)
                <div class="bg-card border border-borderCol rounded-3xl p-6 transition-all duration-300 hover:border-orangeAccent hover:shadow-2xl hover:-translate-y-1 flex flex-col justify-between group shadow-xl">
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-borderCol text-xs">
                        <button type="button" onclick="editSubject({{ $subject->id }}, @js($subject->name))" class="text-textSecondary hover:text-textPrimary font-medium">
                            Edit Name
                        </button>
                        <button type="button" onclick="deleteSubject({{ $subject->id }})" class="text-red-400 hover:text-red-300 font-medium">
                            Delete
                        </button>
                    </div>
                </div>
