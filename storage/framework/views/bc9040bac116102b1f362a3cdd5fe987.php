...
<div class="max-w-[92rem] mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <?php if(session('success')): ?>
        <div id="flashAlert" class="fixed top-6 left-1/2 transform -translate-x-1/2 z-50 bg-emerald-950/95 border border-emerald-500 text-emerald-200 px-8 py-4 rounded-2xl shadow-2xl text-base flex items-center gap-3 transition-all duration-500">
    ...
    <?php if(session('error')): ?>
        <div id="flashAlert" class="fixed top-6 left-1/2 transform -translate-x-1/2 z-50 bg-red-950/95 border border-red-500 text-red-200 px-8 py-4 rounded-2xl shadow-2xl text-base flex items-center gap-3 transition-all duration-500">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 bg-surface border border-borderCol p-8 sm:p-10 rounded-3xl shadow-darkCard">
            <div class="flex flex-col sm:flex-row sm:items-center gap-5">
                ...
                <h1 class="text-3xl sm:text-4xl font-extrabold text-textPrimary tracking-tight break-words flex items-center gap-3">
                    ...
                    <span><?php echo e($subject->name); ?> <span class="text-textMuted font-medium text-xl sm:text-2xl">/ Attendance Report</span></span>
                </h1>
            </div>
        </div>
    </div>
    ...
    <div class="bg-surface border border-borderCol p-3 rounded-2xl mb-8 shadow-darkCard flex flex-wrap items-center gap-3">
        <a
            href="<?php echo e(route('admin.subjects.report', ['id' => $subject->id, 'type' => 'daily', 'date' => $date])); ?>"
        ...
        <a
            href="<?php echo e(route('admin.subjects.report', ['id' => $subject->id, 'type' => 'monthly', 'month' => $month])); ?>"
        ...
        <a
            href="<?php echo e(route('admin.subjects.report', ['id' => $subject->id, 'type' => 'custom', 'start' => $start, 'end' => $end])); ?>"
        ...
        <a
            href="<?php echo e(route('admin.subjects.report', ['id' => $subject->id, 'type' => 'all'])); ?>"
    ...
    <div class="bg-surface border border-borderCol p-6 sm:p-8 rounded-3xl mb-8 shadow-darkCard">
        <form method="GET" action="<?php echo e(route('admin.subjects.report', ['id' => $subject->id, 'type' => 'daily'])); ?>" class="flex flex-col lg:flex-row items-end lg:items-center justify-between gap-5">
            ...
            <div>
                <a
                    href="<?php echo e(route('admin.subjects.pdf', ['id' => $subject->id, 'type' => 'daily', 'date' => $date])); ?>"
                    target="_blank"
    ...
    <div class="bg-red-950/80 border border-red-500/50 text-red-200 px-6 py-5 rounded-2xl mb-8 shadow-lg text-base">
        ...
        <div class="text-sm mt-2 opacity-80">PDF download is not available because no attendance record exists.</div>
    </div>
    <?php endif; ?>
    <?php endif; ?>
    ...
    <?php if($type === 'monthly'): ?>
    <div class="bg-surface border border-borderCol p-6 sm:p-8 rounded-3xl mb-8 shadow-darkCard">
        <form method="GET" action="<?php echo e(route('admin.subjects.report', ['id' => $subject->id, 'type' => 'monthly'])); ?>" class="flex flex-col lg:flex-row items-end lg:items-center justify-between gap-5">
            ...
            <div>
                <a
                    href="<?php echo e(route('admin.subjects.pdf', ['id' => $subject->id, 'type' => 'monthly', 'month' => $month])); ?>"
                    target="_blank"
                    class="btn-danger px-6 py-3 rounded-xl font-semibold text-sm inline-flex items-center gap-2 shadow-md w-full sm:w-auto justify-center"
                >
                    <span>📄</span> Download PDF
    ...
    <?php if($type === 'custom'): ?>
    <div class="bg-surface border border-borderCol p-6 sm:p-8 rounded-3xl mb-8 shadow-darkCard">
        <form method="GET" action="<?php echo e(route('admin.subjects.report', ['id' => $subject->id, 'type' => 'custom'])); ?>" class="flex flex-col lg:flex-row items-end lg:items-center justify-between gap-5" id="customReportForm">
            ...
            <div class="flex items-center gap-3 w-full lg:w-auto justify-end pt-2">
                ...
                <?php if($hasAttendance): ?>
                <a
                    href="<?php echo e(route('admin.subjects.pdf', ['id' => $subject->id, 'type' => 'custom', 'start' => $start, 'end' => $end])); ?>"
                    target="_blank"
    ...
    <div class="bg-surface border border-borderCol p-6 sm:p-8 rounded-3xl mb-8 shadow-darkCard flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div>
            <a
                href="<?php echo e(route('admin.subjects.pdf', ['id' => $subject->id, 'type' => 'all'])); ?>"
                target="_blank"
                class="btn-danger px-6 py-3 rounded-xl font-semibold text-sm inline-flex items-center gap-2 shadow-md justify-center"
            >
                <span>📄</span> Download PDF
    ...
    <div class="bg-red-950/80 border border-red-500/50 text-red-200 px-6 py-5 rounded-2xl mb-8 shadow-lg text-base">
        ...
        <div>No attendance records exist for this subject.</div>
    </div>
    <?php endif; ?>
    <?php endif; ?>
        ...
        <div class="overflow-x-auto">
            <table class="table-dark text-left text-base" id="studentsTable">
                ...
                <tbody id="reportTableBody">
                    ...
                    <tr
                        class="report-row"
                        data-search="<?php echo e(strtolower($data['name'] . ' ' . $data['roll'] . ' ' . $data['branch'] . ' ' . $data['phone'])); ?>"
                    >
