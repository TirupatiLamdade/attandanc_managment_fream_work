...
<?php $__env->startSection('title', $subject->name . ' - Students'); ?>
    ...
    <div class="mb-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-5 bg-surface border border-borderCol p-6 sm:p-8 rounded-3xl shadow-darkCard">
            <div class="flex flex-col sm:flex-row sm:items-center gap-5">
                <a href="<?php echo e(route('admin.subjects.show', $subject->id)); ?>" class="btn-outline inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold shrink-0 w-fit">
                    &larr; Back
                </a>
                <div>
            ...
            <table class="table-dark text-left text-base min-w-[1000px]" id="studentsTable">
                ...
                <tbody id="studentTableBody">
                    <?php $__empty_1 = true; $__currentLoopData = $students; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $student): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="student-row hover:bg-cardHover/50 transition-colors" data-name="<?php echo e(strtolower($student->name)); ?>" data-roll="<?php echo e(strtolower($student->roll_number)); ?>" data-branch="<?php echo e(strtolower($student->branch)); ?>" data-phone="<?php echo e(strtolower($student->phone)); ?>">
                            ...
                            <td class="font-bold text-textPrimary text-lg py-5 px-6"><?php echo e($student->name); ?></td>
                            <td class="py-5 px-6">
                                <span class="bg-card px-3 py-1.5 rounded-xl border border-borderCol text-sm font-mono font-semibold"><?php echo e($student->roll_number); ?></span>
                            </td>
                            <td class="text-textSecondary py-5 px-6"><?php echo e($student->branch); ?></td>
                            <td class="text-textSecondary font-mono py-5 px-6"><?php echo e($student->phone); ?></td>
                            <td class="py-5 px-6 text-center">
                                <div class="flex items-center justify-center gap-3">
                                    <button type="button" title="View Student Added Date" onclick="openStudentDateModal(<?php echo \Illuminate\Support\Js::from($student->name)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($student->roll_number)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($student->created_at ? $student->created_at->format('d/m/Y') : 'Date not available')->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($student->created_at ? $student->created_at->format('h:i A') : '')->toHtml() ?>)" class="p-2.5 rounded-xl text-xs font-bold bg-emerald-600/20 border border-emerald-500/40 text-emerald-400 hover:bg-emerald-600/30 transition-all inline-flex items-center justify-center cursor-pointer">📅</button>
                                    <button type="button" onclick="openEditModal(<?php echo \Illuminate\Support\Js::from($student->id)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($student->name)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($student->roll_number)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($student->branch)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($student->phone)->toHtml() ?>)" class="px-4 py-2.5 rounded-xl text-xs font-bold bg-blue-600/20 border border-blue-500/40 text-blue-400 hover:bg-blue-600/30 transition-all cursor-pointer">Edit</button>
                                    <button type="button" onclick="openDeleteModal(<?php echo \Illuminate\Support\Js::from($student->id)->toHtml() ?>)" class="px-4 py-2.5 rounded-xl text-xs font-bold bg-red-600/20 border border-red-500/40 text-red-400 hover:bg-red-600/30 transition-all cursor-pointer">Delete</button>
                                </div>
                            </td>
                        </tr>
...
<div id="addModal" class="hidden fixed inset-0 bg-black/85 backdrop-blur-md items-center justify-center z-50 p-4 overflow-y-auto">
    <div class="bg-surface border border-borderCol rounded-3xl p-8 w-full max-w-lg shadow-darkCard">
        ...
        <form id="addStudentForm" method="POST" action="<?php echo e(route('admin.subjects.students.store', $subject->id)); ?>" class="space-y-5" onsubmit="return validateStudentForm(event, this)">
            <?php echo csrf_field(); ?>
            <div>
                <label class="block text-xs font-semibold text-textSecondary uppercase tracking-wider mb-2">Student Name *</label>
                <input type="text" name="name" id="addName" required maxlength="255" placeholder="Enter student name" value="<?php echo e(old('name')); ?>" class="input-dark rounded-2xl px-5 py-3.5 text-base w-full transition-colors" oninput="validateFieldLive(this, 'name')">
            ...
            <div>
                ...
                <input type="text" name="roll_number" id="addRoll" required maxlength="50" placeholder="Enter roll number" value="<?php echo e(old('roll_number')); ?>" class="input-dark rounded-2xl px-5 py-3.5 text-base w-full transition-colors" oninput="validateFieldLive(this, 'roll')">
            ...
            <div>
                ...
                <input type="text" name="branch" id="addBranch" required maxlength="255" placeholder="Example: Computer Engineering" value="<?php echo e(old('branch')); ?>" class="input-dark rounded-2xl px-5 py-3.5 text-base w-full transition-colors" oninput="validateFieldLive(this, 'branch')">
            ...
            <div>
                ...
                <input type="tel" name="phone" id="addPhone" required maxlength="10" minlength="10" placeholder="Enter 10 digit mobile number" value="<?php echo e(old('phone')); ?>" class="input-dark rounded-2xl px-5 py-3.5 text-base w-full transition-colors" oninput="validateFieldLive(this, 'phone')">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        <?php $__currentLoopData = $subject->students; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $st): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\Attandance_Fremwork_2026\resources\Views/admin/subjects/students.blade.php ENDPATH**/ ?>